...
function [varargout]  = EigenGP_Upd_kerB_B_UI(x, y, xs, ys, M, opt)
[N,D] = size(x);
...
param = reshape(opt.B,M*D,1);
[new_param] = minimize(param, @(param) EigenGP_Upd_kerB_B(param, opt.lik, opt.cov, x, y, M), opt.nIter);
opt.B = reshape(new_param,M,D);

%save('demo.mat');

[nlZ mu S2] = EigenGP_Upd_kerB_B(new_param, opt.lik, opt.cov, x, y, M, xs);
post.nlZ = nlZ;
post.opt = opt;
%mu = mu + meanp;
if isempty(ys)
