...
function [varargout] = Nystrom_gradient(param, x, y, M, xs)
[N,D] = size(x);
...
elseif nargout == 2
    ...
    while nact<Ns
        ...
        %Ks = covSEard(cov, x, xs(id,:));
        %Kss = covSEard(cov, xs(id,:), 'diag');
        Ks = sf2*exp(-0.5*sq_dist(scale_cols(x,inv_ell)', scale_cols(xs(id,:),inv_ell)'));
        ...
        % Predictive mean
        mu(id) = Ks' * invCN_y;
        S2(id) = sf2 + sn2 - sum(Ks.*(Ks - Kxb*(iKbb_Kbx_iCN*Ks)/sn2),1)';
        % Predictive variance
