...
function [varargout]  = Nystrom_gradient_UI(x, y, xs, ys, M, opt)
[N,D] = size(x);
...
[new_param] = minimize(param, @(param) Nystrom_gradient(param, x, y, M), opt.nIter);
opt.lik = new_param(1);
...
[nlZ mu S2] = Nystrom_gradient(new_param, x, y, M, xs);
post.nlZ = nlZ;
post.opt = opt;
%mu = mu + meanp;
if isempty(ys)
